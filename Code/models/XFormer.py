...
class XFormer(TwoTowerBaseModel):
    def __init__(self, manager):
        ...
        if self.bert_name == "reformer":
            ...
            config.axial_pos_shape = (32,20)
            self.bert = ReformerModel(config)
    ...
    def encode_news(self, x):
        ...
        if self.bert_name in ["reformer"]:
            ...
            cdd_news_repr = self.pooler(self.bert(cdd_news, cdd_attn_mask)[0][:, 0])
        else:
            cdd_news = cdd_news.view(-1, self.signal_length)
    ...
    def encode_user(self,x):
        ...
        if self.bert_name in ["reformer"]:
            user_repr = self.pooler(self.bert(his_news, his_attn_mask)[0][:, 0])
        else:
